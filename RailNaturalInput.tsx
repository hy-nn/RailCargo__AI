...
const recommendedKeywords = ["오봉행 화물", "냉동 컨테이너", "긴급 배송"];
...
export const RailNaturalInput = (): JSX.Element => {
  const navigate = useNavigate();
  const [inputText, setInputText] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const monthlySavings = getMonthlySavingsKgCO2e();
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const parsed = await parseRequest(inputText, "RAIL_ONLY");
      ...
      console.log("candidateTrains:", candidateTrains);
      ...
      window.sessionStorage.setItem("railBookingDraft", JSON.stringify(payload));
      navigate("/05_rail_recommend", { state: payload });
    } catch (error) {
      console.error(error);
      ...
      <div className="mx-auto flex w-full max-w-[1240px] flex-col gap-6 px-6 py-8">
        <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-2">
            <h1 className="text-[33px] font-extrabold leading-[1.3] tracking-[-0.02em] text-[#0b1c30]">
              AI로 내 화물에 맞는 최적의 철도 운송을 찾아보세요.
            </h1>
            <p className="text-[18px] font-bold leading-[1.6] text-[#424751]">
              화물 스케줄과 조건에 맞춰 최적화된 Station-to-Station 직송
              솔루션을 제공합니다.
            </p>
          </div>
          ...
          <Card className="w-full max-w-[330px] rounded-2xl border border-[#d4dbe6] bg-white shadow-[0_2px_6px_rgba(0,0,0,0.04)]">
            <CardContent className="flex flex-col items-center justify-center text-center gap-1.5 p-6">
              ...
              <p className="whitespace-nowrap text-[38px] font-extrabold leading-none text-[#005bac]">
                {Math.round(monthlySavings * 10) / 10} kg
              </p>
          ...
          <CardContent className="space-y-4 p-6">
            ...
            >
              <input
                ...
                placeholder="예) 부산신항의 물건을 군포에 있는 제 창고로 8월 22일까지 보내고 싶어요."
              />
              <Button
                type="submit"
                disabled={isLoading}
