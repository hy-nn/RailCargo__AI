...
export const RailRoadNaturalInput = (): JSX.Element => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const parsed = await parseRequest(inputText, "RAIL_TRUCKER");
      ...
      if (!parsed.finalDestination?.city || !parsed.finalDestination.city.trim()) {
        setErrorMessage(
          "철도+도로 서비스는 최종 배송지의 도시명(예: 군포, 수원 등)이 필요합니다. 목적지 도시를 포함해서 다시 요청해주세요."
        );
        return;
      }
      ...
      window.sessionStorage.setItem("railRoadBookingDraft", JSON.stringify(payload));
      navigate("/08_road_recommend", { state: payload });
    } catch (error) {
      console.error(error);
          ...
          <CardContent className="space-y-4 p-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {recommendedKeywords.map((keyword) => (
                <Badge
                  key={keyword}
                  className="cursor-pointer rounded-full border-0 bg-[#e8f1ff] px-3 py-1 text-[13px] font-extrabold text-[#005bac] hover:bg-[#d0e2ff]"
