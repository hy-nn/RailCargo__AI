...
export const RailRoadRecommended = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      window.sessionStorage.setItem("railRoadBookingDraft", JSON.stringify(state));
    } else {
      setErrorMessage("입력 정보를 찾을 수 없습니다. 다시 요청해주세요.");
      const timer = setTimeout(() => {
        navigate("/07_road_nlp_input");
      }, 3000);
    ...
    >
      ...
      <div className="flex w-full max-w-[1240px] flex-col px-6">
        <SectionAnalysisSubsection
          ...
          backRoutePath="/07_road_nlp_input"
          showStationMappingNotice={true}
        />
        <SectionRecommendedSubsection
          mappedResult={data.mappedResult}
          candidateTrains={data.candidateTrains}
          nextRoutePath="/09_road_confirm"
          sessionStorageKey="railRoadBookingDraft"
        />
      </div>
    </main>
  );
